...
void CalibPHYSICS::Print()
{
	...
	if (boolRunList)
		printf("List of used runs: %s\n", fileRunList.data());
	else
		printf("No list of runs specified.\n");
	if (boolGeometry)
	...
	if (boolIdedx)
		printf("Incomming ion energy loss tables: %s\n", fileIdedx.data());
	else
		printf("No incoming ion energy loss tables specified.\n");
	if (boolLdedx)
		printf("Target-like particle energy loss tables: %s\n", fileLdedx.data());
	else
		printf("No target-like energy loss tables specified.\n");
	if (boolHdedx)
		printf("Beam-like particle energy loss tables: %s\n", fileHdedx.data());
	else
		printf("No beam-like particle energy loss tables specified.\n");
	if (boolFGate)
		printf("YY1/CsI gate in %s\n", fileGate.data());
	else
		printf("No YY1/CsI gate file specified.\n");
	if (boolNGate)
		printf("YY1/CsI gate name: %s\n", nameGate.data());
	else
		printf("No YY1/CsI gate name specified.\n");
	printf("Using CsI%d for gate.\n", numGate);
	if (boolFSdGate)
		printf("S3 gate in %s\n", fileSdGate.data());
	else
		printf("No S3 gate file specified.\n");
	if (boolNSdGate)
		printf("S3 gate name: %s\n", nameSdGate.data());
	else
		printf("No S3 gate name specified.\n");
	if (boolFYuGate)
		printf("Upstream YY1 gate in %s\n", fileYuGate.data());
	else
		printf("No Upstream YY1 gate file specified.\n");
	if (boolNYuGate)
		printf("Upstream YY1 gate name: %s\n", nameYuGate.data());
	else
		printf("No upstream YY1 gate name specified.\n");
	if (boolFSuGate)
		printf("Upstream S3 gate in %s\n", fileSuGate.data());
	else
		printf("No upstream S3 gate file specified.\n");
	if (boolNSuGate)
		printf("Upstream S3 gate name: %s\n", nameSuGate.data());
	else
		printf("No upstream S3 gate name specified.\n");
	if (boolRunDepPar)
		printf("Run dependant parameters: %s\n", fileRunDepPar.data());
	else
		printf("No run dependant parameters specified.\n");
	if (boolIC)
		printf("Isobutane in ionization chamber.\n");
	else
		printf("No isobutane in ionization chamber.\n");
	if (boolICGates)
		printf("Using ionization chamber for incoming ID.\n");
	else
		printf("No incoming ID using the ionization chamber.\n");
	printf("********************************\n\n");
}
