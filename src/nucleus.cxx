...
void nucleus::ReadFile(std::string binpath, int inputA, std::string inpEl)
{
	...
	while (!feof(rctfile1))
	{
		...
		if (cS2n == "*")
			...
			sscanf(cS2n.data(), "%lf", &S2n);
		if (cS2p == "*")
			S2p = 0.;
		else
			sscanf(cS2p.data(), "%lf", &S2p);
	...
	while (!feof(rctfile2))
	{
		...
		if (cSn == "*")
			...
			sscanf(cSn.data(), "%lf", &Sn);
		if (cSp == "*")
			Sp = 0.;
		else
			sscanf(cSp.data(), "%lf", &Sp);
