...
TChain *createChain(std::vector<Int_t> runs, std::string Directory)
{
	...
	for (int i = 0; i < nRuns; ++i)
	{
		...
		if (runs.at(i) == 0)
			runNo = "";
		else if (runs.at(i) > 99999)
			runNo = Form("%d", runs.at(i));
		else
			runNo = Form("0%04d", runs.at(i));
...
void calculateBeamEnergy(nucleus A, Double_t E)
{
	...
	{ // foil facing downstream of target
		...
		if (geoP.FoilThickness > 0.)
			E = E - eloss(A, 1., E, geoP.FoilThickness, IrisMaterial::Foil);
		else
			E = temp_E;
	...
	{ // foil facing upstream of target
		if (geoP.FoilThickness > 0.)
			E = E - eloss(A, 1., E, geoP.FoilThickness, IrisMaterial::Foil);
		else
			E = temp_E;
...
void HandleBOR_PHYSICS(std::string BinPath, std::string Directory, std::string CalibFile, std::string OutputFile)
{
	...
	else
	{
		...
		while (fgets(rLine, 256, rFile) != NULL)
		{
			...
			if (runs.size() == 0 || runTmp > runs.at(runs.size() - 1))
	...
	if (input_chain->GetListOfBranches()->FindObject("Evnt"))
		input_chain->SetBranchStatus("Evnt", 0);
	if (input_chain->GetListOfBranches()->FindObject("reacX"))
		input_chain->SetBranchStatus("reacX", 0);
	if (input_chain->GetListOfBranches()->FindObject("reacY"))
		input_chain->SetBranchStatus("reacY", 0);
	if (input_chain->GetListOfBranches()->FindObject("reacZ"))
		input_chain->SetBranchStatus("reacZ", 0);
	if (input_chain->GetListOfBranches()->FindObject("wght"))
		input_chain->SetBranchStatus("wght", 0);
	if (input_chain->GetListOfBranches()->FindObject("Qgen"))
		input_chain->SetBranchStatus("Qgen", 0);
	if (input_chain->GetListOfBranches()->FindObject("qdet"))
		input_chain->SetBranchStatus("Qdet", 0);
	if (input_chain->GetListOfBranches()->FindObject("ICdE"))
		input_chain->SetBranchStatus("ICdE", 0);
	if (input_chain->GetListOfBranches()->FindObject("SSBdE"))
		input_chain->SetBranchStatus("SSBdE", 0);
	if (input_chain->GetListOfBranches()->FindObject("yd"))
		input_chain->SetBranchStatus("yd*", 0);
	if (input_chain->GetListOfBranches()->FindObject("csi"))
		input_chain->SetBranchStatus("csi*", 0);
	if (input_chain->GetListOfBranches()->FindObject("sd1"))
		input_chain->SetBranchStatus("sd1*", 0);
	if (input_chain->GetListOfBranches()->FindObject("sd2"))
		input_chain->SetBranchStatus("sd2*", 0);
	//***************************************************************************
	...
	if (calPhys.boolFGate == kTRUE)
	{
		...
		if (calPhys.boolNGate == kTRUE)
		{
			...
			if (!YdCsIGate)
				printf("No Yd/CsI gate.\n");
			else
				printf("Grabbed Yd/CsI gate %s.\n", calPhys.nameGate.data());
		}
		else
			printf("No Yd/CsI gate.\n");
	}
	...
	if (calPhys.boolFSuGate == kTRUE)
	{
		...
		if (calPhys.boolNSuGate == kTRUE)
		{
			...
			if (!SuGate)
				printf("No upstream S3 gate.\n");
			else
				printf("Grabbed upstream S3 gate %s.\n", calPhys.nameSuGate.data());
		}
		else
			printf("No upstream S3 gate.\n");
	}
	...
	if (calPhys.boolRunDepPar)
	{
		...
		/*if (calPhys.boolIdedx == kTRUE)
		{
			...
			dedx_i.Print();
			if (dedx_i.boolAg == kTRUE)
				loadELoss(dedx_i.Ag, eAAg, dedxAAg, mA);
			if (dedx_i.boolIso == kTRUE)
				loadELoss(dedx_i.Iso, eAIso, dedxAIso, mA);
			if (dedx_i.boolWndw == kTRUE)
				loadELoss(dedx_i.Wndw, eAWndw, dedxAWndw, mA);
			if (dedx_i.boolTgt == kTRUE)
				loadELoss(dedx_i.Tgt, eATgt, dedxATgt, mA);
		}
		...
		if (calPhys.boolLdedx == kTRUE)
		{
			...
			dedx_l.Print();
			if (dedx_l.boolSi == kTRUE)
				loadELoss(dedx_l.Si, ebSi, dedxbSi, mb);
			if (dedx_l.boolAl == kTRUE)
				loadELoss(dedx_l.Al, ebAl, dedxbAl, mb);
			if (dedx_l.boolB == kTRUE)
				loadELoss(dedx_l.B, ebB, dedxbB, mb);
			if (dedx_l.boolP == kTRUE)
				loadELoss(dedx_l.P, ebP, dedxbP, mb);
			if (dedx_l.boolMy == kTRUE)
				loadELoss(dedx_l.My, ebMy, dedxbMy, mb);
			if (dedx_l.boolTgt == kTRUE)
				loadELoss(dedx_l.Tgt, ebTgt, dedxbTgt, mb);
			if (dedx_l.boolAg == kTRUE)
				loadELoss(dedx_l.Ag, ebAg, dedxbAg, mb);
			if (dedx_l.boolSiO2 == kTRUE)
				loadELoss(dedx_l.SiO2, ebSiO2, dedxbSiO2, mb);
		}
		...
		if (calPhys.boolHdedx == kTRUE)
		{
			...
			dedx_h.Print();
			if (dedx_h.boolTgt == kTRUE)
				loadELoss(dedx_h.Tgt, eBTgt, dedxBTgt, mB);
			if (dedx_h.boolSi == kTRUE)
				loadELoss(dedx_h.Si, eBSi, dedxBSi, mB);
			if (dedx_h.boolAl == kTRUE)
				loadELoss(dedx_h.Al, eBAl, dedxBAl, mB);
			if (dedx_h.boolB == kTRUE)
				loadELoss(dedx_h.B, eBB, dedxBB, mB);
			if (dedx_h.boolP == kTRUE)
				loadELoss(dedx_h.P, eBP, dedxBP, mB);
			if (dedx_h.boolSiO2 == kTRUE)
				loadELoss(dedx_h.SiO2, eBSiO2, dedxBSiO2, mB);
			if (dedx_h.boolAg == kTRUE)
				loadELoss(dedx_h.Ag, eBAg, dedxBAg, mB);
			if (dedx_h.boolMy == kTRUE)
				loadELoss(dedx_h.My, eBMy, dedxBMy, mB);
		}*/

		// Initialize runPar with values from first run in chain
		if (runDepPar.bool_runPar == kTRUE)
			getRunPar(runs.at(0));
		prevRun = runs.at(0);
...
void HandlePHYSICS()
{
	...
	for (Int_t i = 0; i < nEntries; i++)
	{
		...
		if (YdCsIGate != NULL && (det->TYdMul == 0))
			continue; // event has YY1
		if (det->TICEnergy.size() == 0)
			continue;
		if (det->TICEnergy.at(0) < runDepPar.ICmin || det->TICEnergy.at(0) > runDepPar.ICmax)
			continue; // event in IC YdCsIGate?
		for (int z = 0; z < det->TYdMul; z++)
		{
			if (YdCsIGate != NULL && calPhys.numGate != 2 && det->TCsI1Mul > 0 && YdCsIGate->IsInside(det->TCsI1Energy.at(z), det->TYdEnergy.at(z) * cos(TMath::DegToRad() * det->TYdTheta.at(z))) == 1)
				conditionsmet = 1; // event in proton/deuteron/etc YdCsIGate?
			break;
		}
		for (int z = 0; z < det->TYdMul; z++)
		{
			if (YdCsIGate != NULL && calPhys.numGate == 2 && det->TCsI2Mul > 0 && YdCsIGate->IsInside(det->TCsI2Energy.at(z), det->TYdEnergy.at(z) * cos(TMath::DegToRad() * det->TYdTheta.at(z))) == 1)
				conditionsmet = 1; // event in proton/deuteron/etc YdCsIGate?
			break;
		}
		if (SdGate != NULL && (det->TSd1rEnergy.size() == 0 || det->TSd1sEnergy.size() == 0))
			continue; // event has S3 hit?
		for (int z = 0; z < det->TSd1rMul; z++)
		{
			if (SdGate != NULL && det->TSd2rMul == det->TSd1rMul && SdGate->IsInside(det->TSd2rEnergy.at(z), det->TSd1rEnergy.at(z) * cos(TMath::DegToRad() * det->TSd1Theta.at(z))) == 1)
				conditionsmet = 1; // event in proton/deuteron/etc SdGate?
			break;
		}
		if (YuGate != NULL && det->TYuEnergy.size() == 0)
			continue; // event has Yu hit?
		if (YuGate != NULL && YuGate->IsInside(det->TYuTheta.at(0), det->TYuEnergy.at(0)) == 0)
			continue; // event in proton/deuteron/etc YuGate?
		if (SuGate != NULL && (det->TSurEnergy.size() == 0 || det->TSusEnergy.size() == 0))
			continue; // event has Su hit?
		if (SuGate != NULL && SuGate->IsInside(det->TSuTheta.at(0), det->TSurEnergy.at(0)) == 0)
			continue; // event in proton/deuteron/etc SuGate?
		if (YdCsIGate == NULL && SdGate == NULL)
			conditionsmet = 1;
		if (conditionsmet == 0)
			continue;
		...
		{ // check if in the proton/deuteron YdCsIGate
			...
			for (int z = 0; z < det->TYdMul; z++)
			{
				...
				// std::cout << "TYdMul: " << det->TYdMul << "; TCsI1Energy.size(): " << det->TCsI1Energy.size() << "; TCsI1Energy 0: " << det->TCsI1Energy[0] << "; TCsI1Energy 1: " << det->TCsI1Energy.at(1) << std::endl;
		...
		if (det->TYuEnergy.size() > 0 && det->TYuRing.size() > 0)
		{ // check if in the proton/deuteron YuGate
