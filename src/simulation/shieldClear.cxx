...
Bool_t shieldClear(Double_t theta, Double_t phi)
{
  ...
  if (yOnShield < 0)
  {
    ...
    clearCirc = yOnShield > -(verSideDown - cyCurve) || fabs(xOnShield) < (horSide / 2 - cyCurve) ? true
                                                                                                  : pow((fabs(xOnShield) - horSide / 2 + cyCurve), 2) + pow((fabs(yOnShield) - verSideDown + cyCurve), 2) < cyCurve * cyCurve;
  }
  else
  {
    clearRect = ((fabs(xOnShield) < horSide / 2) && (fabs(yOnShield) < verSideUp));
    clearCirc = yOnShield < verSideUp - cyCurve || fabs(xOnShield) < (horSide / 2 - cyCurve) ? true
                                                                                             : pow((fabs(xOnShield) - horSide / 2 + cyCurve), 2) + pow((fabs(yOnShield) - verSideUp + cyCurve), 2) < cyCurve * cyCurve;
  }
