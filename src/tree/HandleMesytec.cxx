...
const int NSd1rChannels = 24;
const int NSd1sChannels = 32;
const int NSd2rChannels = 24;
...
void HandleMesytec(TMidasEvent &event, void *ptr, int nitems, int bank, IDet *pdet, TString CalibFile, bool gUseRaw)
{
		...
		for (i = 0; i < nitems; i++)
		{
			...
			case 0x04000000: // Data[I] Event
				...
				if (modid == 2 && vpeak > adcThresh && vpeak < 3840)
				{
					...
					if (channel < 16)
						ZdyADC[channel] = vpeak;
					if (!usePeds)
					{
						Sd2r[channel] = Sd2rOffset[channel] + Sd2rGain[channel] * ((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5));
						if (channel < 16)
							Zdy[channel] = ZdyOffset[channel] + ZdyGain[channel] * ((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5));
					}
					else if (usePeds)
					{
						Sd2r[channel] = Sd2rGain[channel] * (((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5)) - Sd2rPed[channel]);
						if (channel < 16)
							Zdy[channel] = ZdyGain[channel] * (((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5)) - ZdyPed[channel]);
					}
				}
				...
				if (modid == 3 && vpeak > adcThresh && vpeak < 3840)
				{
					...
					{
						ch = (channel == 0) ? 0 : 32 - channel;
					}
					if (channel < 8)
						ZdxADC[7 - channel] = vpeak;
					if (channel > 7 && channel < 16)
						ZdxADC[channel] = vpeak;
					if (!usePeds)
					{
						Sd2s[ch] = Sd2sOffset[ch] + Sd2sGain[ch] * ((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5));
						if (channel < 8)
							Zdx[7 - channel] = ZdxOffset[7 - channel] + ZdxGain[7 - channel] * ((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5));
						if (channel > 7 && channel < 16)
							Zdx[channel] = ZdxOffset[channel] + ZdxGain[channel] * ((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5));
					}
					else if (usePeds)
					{
						Sd2s[ch] = Sd2sGain[channel] * (((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5)) - Sd2sPed[channel]);
						if (channel < 8)
							Zdx[7 - channel] = ZdxGain[7 - channel] * (((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5)) - ZdxPed[7 - channel]);
						if (channel > 7 && channel < 16)
							Zdx[channel] = ZdxGain[channel] * (((float)vpeak + gRandomise * fRandom.Uniform(-0.5, 0.5)) - ZdxPed[channel]);
					}
				}
				...
				// if (modid>5 && modid<10 && vpeak>adcThresh  && vpeak<3500){
	...
	if (bank == 5)
	{
		...
		// Charge sharing carried out first as Charge in Sd1 affects Charge in Sd2
		...
		for (Int_t i = 0; i < NSd1rChannels; i++)
		{
			...
			if (maxE > 0.)
			{
				...
				if (gUseRaw)
					det.TSd1rADC.push_back(Sd1rADC[maxCh]);
		...
		for (Int_t i = 0; i < NSd1sChannels; i++)
		{
			...
			if (maxE > 0.)
			{
				...
				if (gUseRaw)
					det.TSd1sADC.push_back(Sd1sADC[maxCh]);
		...
		det.TSd2rEnergy.resize(det.TSd1rMul, NaN);
		det.TSd2rChannel.resize(det.TSd1rMul, -1);
		det.TSd2Theta.resize(det.TSd1rMul, NaN);
		// int Sd2rxhit = 0;
		for (Int_t i = 0; i < NSd2rChannels; i++)
		{
			...
			if (maxE > 0.01 && maxE < 3840)
			{
				det.TSd2rMul++;
				if (gUseRaw)
					det.TSd2rADC.push_back(Sd2rADC[maxCh]);
				// det.TSd2rNeighbour.push_back(-1);
				rndm = fRandom.Rndm(); // random number between 0 and 0.99 for each event
				theta = TMath::RadToDeg() * atan((geoM.SdInnerRadius * (24. - maxCh - rndm) + geoM.SdOuterRadius * (maxCh + rndm)) / 24. / (geoM.Sd1Distance + 14.8));
				if (det.TSd1rMul > 0)
				{
					for (Int_t k = 0; k < det.TSd1rMul; k++)
					{
						if ((maxCh - det.TSd1rChannel.at(k)) >= 0 && (maxCh - det.TSd1rChannel.at(k)) <= 2)
						{ // Sd1rChannel match Sd2rChannel+2
							if (std::isnan(det.TSd2rEnergy.at(k)))
							{ // Sd1rChannel match Sd2rChannel+2. Check if there is already an energy in the hit
								det.TSd2rEnergy.at(k) = Sd2r[maxCh];
								det.TSd2rChannel.at(k) = maxCh;
								det.TSd2Theta.at(k) = theta;
								Sd2r[maxCh] = 0.;
								Sd2rADC[maxCh] = 0;
								break;
							}
				...
				if (Sd2r[maxCh] != 0)
				{ // Sd1rChannel does not match Sd2rChannel+2. Place energy in TSd1rhit++;
					det.TSd2rEnergy.push_back(Sd2r[maxCh]);
					det.TSd2rChannel.push_back(maxCh);
					det.TSd2Theta.push_back(theta);
		...
		det.TSd2sEnergy.resize(det.TSd1sMul, NaN);
		det.TSd2sChannel.resize(det.TSd1sMul, -1);
		det.TSd2Phi.resize(det.TSd1sMul, NaN);
		// int Sd2sxhit = 0;
		for (Int_t i = 0; i < NSd2sChannels; i++)
		{
			...
			if (maxE > 0.01 && maxE < 3840)
			{
				det.TSd2sMul++;
				if (gUseRaw)
					det.TSd2sADC.push_back(Sd2sADC[maxCh]);
				/// det.TSd2sNeighbour.push_back(-1);
				phi = 180. - 360. * maxCh / 32.;
				rndm = fRandom.Rndm(); // random number between 0 and 0.99 for each event
				if (det.TSd1sMul > 0)
				{
					for (Int_t k = 0; k < det.TSd1sMul; k++)
					{
						if ((maxCh - det.TSd1sChannel.at(k)) == -1 || (maxCh - det.TSd1sChannel.at(k)) == 0 || (maxCh - det.TSd1sChannel.at(k)) == 1 || (maxCh - det.TSd1sChannel.at(k)) == 31 || (maxCh - det.TSd1sChannel.at(k)) == -31)
						{ // Check if Sd1sChannel matches
							if (std::isnan(det.TSd2sEnergy.at(k)))
							{ // Sd1sChannel match Sd2sChannel. Check if there is already an energy in the hit
								det.TSd2sEnergy.at(k) = Sd2s[maxCh];
								det.TSd2sChannel.at(k) = maxCh;
								det.TSd2Phi.at(k) = (phi - 11.25 * rndm);
								Sd2s[maxCh] = 0.;
								Sd2sADC[maxCh] = 0;
								break;
							}
				...
				if (Sd2s[maxCh] != 0)
				{ // Sd1sChannel does not match Sd2rChannel of if hit not assigned. Place energy in TSd1sMul++;
		...
		for (Int_t i = 0; i < NYdChannels; i++)
		{
			...
			if (maxE > 0.)
			{
				...
				if (gUseRaw)
					det.TYdADC.push_back(YdADC[maxCh]);
				...
				phi = 90. + 1.75 - 360. * (maxCh / 16) / 8.;
				rndm = 44.8 * fRandom.Rndm();
				phi = phi - 22.4 + rndm;
				phi = (phi < -180.) ? phi + 360. : phi;
				det.TYdPhi.push_back(phi);
		...
		for (Int_t i = 0; i < NZdxChannels; i++)
		{
			...
			if (maxE > 0.)
			{
				...
				if (gUseRaw)
					det.TZdxADC.push_back(ZdxADC[maxCh]);
		...
		for (Int_t i = 0; i < NZdyChannels; i++)
		{
			...
			if (maxE > 0.)
			{
				...
				if (gUseRaw)
					det.TZdyADC.push_back(ZdyADC[maxCh]);
		...
		// CsI
		det.TCsI1Energy.resize(det.TYdMul, NaN);
		det.TCsI2Energy.resize(det.TYdMul, NaN);
		det.TCsI1Channel.resize(det.TYdMul, -1);
		det.TCsI1Phi.resize(det.TYdMul, NaN);
		for (Int_t i = 0; i < NCsIChannels; i++)
		{
			...
			if (maxE > 0.01)
			{
				if (det.TYdMul > 0)
				{
					for (Int_t l = 0; l < det.TYdMul; l++)
					{
						if (((maxCh / 2) - det.TYdNo.at(l)) == 0)
						{ // checks whether CsI sector matches the Yd Sector
							det.TCsI1Mul++;
							...
							if (gUseRaw)
								det.TCsI1ADC.push_back(CsI1ADC[maxCh]);
				...
				{ // We cannot calibrate the signal, but ADC value is still stored
		...
		det.TCsI2Energy.resize(det.TYdMul, NaN);
		det.TCsI2Channel.resize(det.TYdMul, -1);
		det.TCsI2Phi.resize(det.TYdMul, NaN);
		for (Int_t i = 0; i < NCsIChannels; i++)
		{
			maxE = 0.;
			maxCh = 0;
			...
			if (maxE > 0.01)
			{
				if (det.TYdMul > 0)
				{
					for (Int_t l = 0; l < det.TYdMul; l++)
					{
						...
						{ // checks whether CsI sector matches the Yd Sector
							det.TCsI2Mul++;
							int m = (det.TYdChannel.at(l) % 16) / (16 / NCsI2Group);
							det.TCsI2Energy.at(l) = (CsI2[maxCh] - CsI2Ped[m][maxCh]) * CsI2Gain[m][maxCh];
							det.TCsI2Channel.at(l) = maxCh;
							if (gUseRaw)
								det.TCsI2ADC.push_back(CsI2ADC[maxCh]);
							phi = 90. + 1.75 - 360. * maxCh / 16.;
							rndm = 22.4 * fRandom.Rndm();
							phi = phi - 11.2 + rndm;
							phi = (phi < -180.) ? phi + 360. : phi;
							det.TCsI2Phi.at(l) = phi;
						}
					}
				}
				else
		...
		for (Int_t i = 0; i < NSurChannels; i++)
		{
			...
			if (maxE > 0.)
			{
				...
				if (gUseRaw)
					det.TSurADC.push_back(SurADC[maxCh]);
		...
		for (Int_t i = 0; i < NSusChannels; i++)
		{
			...
			if (maxE > 0.)
			{
				...
				if (gUseRaw)
					det.TSusADC.push_back(SusADC[maxCh]);
		...
		for (Int_t i = 0; i < NYuChannels; i++)
		{
			...
			if (maxE > 0.)
			{
				...
				if (gUseRaw)
					det.TYuADC.push_back(YuADC[maxCh]);
...
void Calibrate_Mesytec(int run) {
	...
	else if (fgets(lineBuff, sizeof(lineBuff), pFile) != NULL)
	{
		printf("Reading IC config file '%s'\n", calMesy.fileIC.data());
		for (int i = 0; i < NICChannels; i++)
		{
			if (fscanf(pFile, "%d%lf%lf", &Chan, &a, &b) == 3)
			{
				ICPed[Chan] = a;
				ICGain[Chan] = b;
	...
	{
		printf("Calibration file for IC exists but is empty. Skipping IC calibration.\n");
	}
	...
	{
		printf("Calibration file for TRIFIC exists but is empty. Skipping TRIFIC calibration.\n");
	}
	//*****************************************************************
	...
	else if (fgets(lineBuff, sizeof(lineBuff), pFile) != NULL)
	{
		...
		for (int i = 0; i < 16; i++)
		{
			for (int j = 0; j < NCsI1Group; j++)
			{
				if (fscanf(pFile, "%d%d%lf%lf", &Chan, &g, &a, &b) == 4)
				{
					CsI1Ped[g][Chan - 32] = a;
	...
	{
		printf("Calibration file for CsI1 exists but is empty. Skipping CsI1 calibration.\n");
	}
	...
	{
		printf("Calibration file for CsI2 exists but is empty. Skipping CsI2 calibration.\n");
	}
	...
	{
		printf("Calibration file for Sd2 rings exists but is empty. Skipping Sd2r calibration.\n");
	}
	//************************************************************************
	...
	{
		printf("Calibration file for Sd2 sectors exists but is empty. Skipping Sd2s calibration.\n");
	}
	...
	{
		printf("Calibration file for Sd1 rings exists but is empty. Skipping Sd1r calibration.\n");
	}
	//************************************************************************
	...
	{
		printf("Calibration file for Sd1 sectors exists but is empty. Skipping Sd1s calibration.\n");
	}
	//************************************************************************
	...
	{
		printf("Calibration file for Su rings exists but is empty. Skipping Sur calibration.\n");
	}
	//************************************************************************
	...
	{
		printf("Calibration file for Su sectors exists but is empty. Skipping Sus calibration.\n");
	}
	//************************************************************************
	...
	{
		printf("Calibration file for Yd exists but is empty. Skipping Yd calibration.\n");
	}
	//************************************************************************
	//**************** Calibrate Yu ****************************************
	Chan = -1;
	...
	{
		printf("Calibration file for Yu exists but is empty. Skipping Yu calibration.\n");
	}
	//************************************************************************
	...
	{
		printf("Calibration file for Zd horizontal strips exists but is empty. Skipping Zdy calibration.\n");
	}
	//************************************************************************
	...
	{
		printf("Calibration file for Zd vertical strips exists but is empty. Skipping Zdx calibration.\n");
	}
