...
void HandleV1190(TMidasEvent &event, void *ptr, int nitems, int bank, ITdc *ptdc)
{
	...
	{
		if (debug)
			printf("V1190_ Evt#:%d nitems:%d bk#%d\n", event.GetSerialNumber(), nitems, bank);
		for (i = 0; i < nitems; i++)
		{
			if (debug)
				printf("items:%d - data[%d]: %d / 0x%x\n", Nvchannels, i, data[i], data[i]);
			uint16_t globCh;
			...
			case 0x40000000: // Global Header
				...
				if (debug)
					printf("Global TDC header. Event counter: %d, GEO: %d\n", evtcnt, geo);
				break;
			case 0x08000000: // TDC Header
				...
				if (debug)
					printf("  0x%08x: ", data[i]);
				if (debug)
					printf("TDC header. TDC: %d, evtid: %d, bunch ID: %d\n", tdc, evtid, bunchid);
				break;
			case 0x00000000: // TDC measurement
				...
				if (debug)
					printf(" TDC data. vchannel: %d, ", vchannel);
			...
			case 0x18000000: // TDC Trailer
				...
				if (debug)
					printf("    0x%08x: ", data[i]);
				if (debug)
					printf("TDC trailer. evtid: %d, wordcnt: %d\n", evtid, wordcnt);
				break;
...
vector<array<double, 2>> ReadCalibrationFile(string fileName)
{
	...
	while (getline(inFile, line))
	{
		...
		if (sscanf(line.c_str(), "%lf %lf %lf", &slope, &delay, &dummy) != 2)
			continue;
...
{
	bool ok = true;
	if (det == "YD" && cal.size() != 128)
		ok = false;
	if (det == "SD1R" && cal.size() != 24)
		ok = false;
	if (det == "SD1S" && cal.size() != 32)
		ok = false;
	if (det == "SD2R" && cal.size() != 24)
		ok = false;
	if (det == "SD2S" && cal.size() != 32)
		ok = false;
	if (det == "YU" && cal.size() != 128)
		ok = false;
	if (det == "SUR" && cal.size() != 24)
		ok = false;
	if (det == "SUS" && cal.size() != 32)
		ok = false;
